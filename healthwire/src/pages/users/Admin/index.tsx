...
const Admin = () => {
  ...
  const fetchUsersData = (page) => {
    axios.get(`https://api.holisticare.pk/apis/user/get?page=${page}&role=administrator`).then((res) => {
      const administrators = res.data.data.filter(user => user.role === "administrator");
