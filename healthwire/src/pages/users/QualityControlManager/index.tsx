...
const QualityControlManager = () => {
  ...
  const fetchUsersData = (page: number) => {
    axios
      .get(`${Base_url}/apis/user/get`, {
        params: { page, limit: 10, role: 'quality_control_manager', ...buildAxiosBranchScopedParams() },
      })
      .then((res) => {
  ...
  const handleEdit = (record: any) => {
    navigate(`/quality-control-manager/edit_user/${record._id}`);
  };
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="mb-5 flex justify-between items-center">
        <h1 className="text-xl font-semibold text-black dark:text-white">Quality Control Manager</h1>
        {allowCreate && (
          <Link
            to="/quality-control-manager/new_user"
