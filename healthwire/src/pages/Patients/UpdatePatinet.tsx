...
const UpdatePatient = ({
  ...
  closeModal,
  fetchPatientData,
  patientData // Add patientData prop to receive the patient to be updated
}) => {
...
console.log(selectedImage);
  ...
  useEffect(() => {
    if (isModalOpen && patientData) {
      // Set form fields with patient data when modal opens
      setMrNumber(patientData.mr || '');
      setName(patientData.name || '');
      setPhone(patientData.phone || '');
      setGender(patientData.gender || '');
      setDob(patientData.dob ? patientData.dob.split('T')[0] : '');
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      console.log('selectedImage after file select:', file);
    }
  };
 ...
 const handleSubmit = async (e) => {
  ...
  if (!name) {
    ...
    toast.error('Must select gender');
    ...
    // Include MR number in the update
    ...
    if (cnic) updatedPatient.append('cnic', cnic.replace(/\D/g, ''));
                ...
                <div className=" text-center my-2">
                  {selectedImage ? (
                    <img
                      src={
                        typeof selectedImage === 'string'
                          ? selectedImage.startsWith('http')
                            ? selectedImage
                            : `${Base_url}/${selectedImage.replace(/^\/+/, '')}`
                      ...
                      alt="Patient"
                      onError={e => { (e.target as HTMLImageElement).src = profile2; }}
                    />
                  ) : (
