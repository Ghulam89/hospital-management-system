...
import { SearchOutlined, PlusOutlined, EditOutlined, DeleteOutlined, EyeOutlined, DownloadOutlined, PrinterOutlined, UndoOutlined } from '@ant-design/icons';
...
const SaleReturn: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      title: 'Customer',
      key: 'customer',
      render: (text: any, record: SaleReturn) => (
        <div>
    ...
    },
    {
      ...
      render: (text: any, record: SaleReturn) => (
        <div>
          <div className="font-semibold text-gray-800">{record.itemName}</div>
    ...
    },
    {
      ...
      key: 'returnQuantity',
      render: (qty: number, record: SaleReturn) => (
        <div>
          <span className="font-semibold text-red-600">{qty}</span>
          <div className="text-sm text-gray-500">of {record.quantity}</div>
        </div>
      ),
    },
    {
  ...
  const handleView = (record: SaleReturn) => {
    Swal.fire({
      ...
      html: `
        <div class="text-left">
          ...
          <p><strong>Return Quantity:</strong> ${record.returnQuantity} of ${record.quantity}</p>
          <p><strong>Unit Price:</strong> Rs. ${record.unitPrice.toLocaleString()}</p>
  ...
  const handleEdit = (record: SaleReturn) => {
    form.setFieldsValue({
      ...record,
  ...
  const handleDelete = async (record: SaleReturn) => {
    try {
      const result = await Swal.fire({
        title: 'Delete Sale Return?',
        text: `Are you sure you want to delete this sale return record?`,
        icon: 'warning',
      ...
      if (result.isConfirmed) {
        message.success('Sale return record deleted successfully');
        fetchSaleReturns();
      }
    } catch (error) {
      console.error('Error deleting sale return:', error);
      message.error('Failed to delete sale return record');
    }
  };
  ...
  const handleModalSubmit = async () => {
    try {
      ...
      const data = {
        ...
        totalAmount: values.returnQuantity * values.unitPrice,
        createdBy: 'current-user-id',
      };

      message.success('Sale return saved successfully');
  ...
  const totalRefundAmount = saleReturns.reduce((sum, ret) => sum + ret.totalAmount, 0);
  const pendingReturns = saleReturns.filter(ret => ret.status === 'Pending').length;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-blue-600">Sale Return</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <Card className="bg-orange-50 border-orange-200">
            <Statistic
              ...
              prefix={<UndoOutlined className="text-orange-500" />}
        ...
        {/* No Data Message */}
        {!loading && saleReturns.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-500 text-lg mb-4">No sale return records found</div>
            <div className="text-gray-400 text-sm">
              Click "Add Sale Return" to record your first sale return
            </div>
          </div>
        )}
        ...
        title={
          <div className="flex items-center">
            ...
            <span className="text-lg font-semibold text-gray-800">
              {form.getFieldValue('_id') ? 'Edit Sale Return' : 'Add Sale Return'}
            </span>
          </div>
        }
        open={isModalOpen}
      ...
      >
        <div className="bg-gray-50 p-4 rounded-lg mb-4">
          <p className="text-sm text-gray-600 mb-0">
            {form.getFieldValue('_id') ? 'Update the sale return information below.' : 'Fill in the details to record sale return.'}
          </p>
        </div>
        ...
        <Form form={form} layout="vertical" className="pharmacy-form">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Form.Item
              ...
              label={
                <span className="font-semibold text-gray-700">
                  Original Quantity <span className="text-red-500">*</span>
                </span>
              }
              rules={[{ required: true, message: 'Please enter original quantity' }]}
            >
              <Input 
                type="number" 
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: 'Please enter return quantity' }]}
            >
              <Input 
                type="number" 
          ...
          <div className="bg-blue-50 p-3 rounded-lg">
            <div className="flex items-center text-sm text-blue-700">
              ...
              <span>Total refund amount will be calculated automatically based on return quantity and unit price.</span>
            </div>
          </div>
        </Form>
      </Modal>
