...
const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (id) {
          const patientRes = await axios.get(`${Base_url}/apis/patient/get/${id}`);
          const patientData = patientRes.data.data;
          setGetPatientData(patientData);
          setPatientInfo(patientData);
          setSearchTerm(`${patientData.name} (MR# ${patientData.mr})`);
  ...
  try {
    ...
    return {
      options: data.map((item: Procedure) => ({
        value: item._id,
        label: `${item.name} (Rs. ${item.amount})`,
        procedureData: item
      })),
  ...
  setProcedures(prevProcedures => {
    return prevProcedures.map(item => {
      if (item.id === id) {
        const updatedItem = { ...item, [field]: value };

        // When procedureId changes, update all related fields
        if (field === 'procedureId') {
          ...
          if (selectedProcedure) {
            ...
            updatedItem.amount = selectedProcedure.amount * item.quantity;
            
            // Reset discount-related fields when procedure changes
            updatedItem.discount = 0;
            updatedItem.discountType = 0;
            updatedItem.deductDiscount = 'Hospital & Doctor';
          }
        }
        ...
        // Always recalculate shares after any field change
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
        ...
        response = await axios.post(`${Base_url}/apis/invoice/create`, invoicePayload);
        if (response.data.status === "ok") {
          toast.success('Invoice created successfully!');
          
          navigate(`/invoice/patient/${id}`);
        }
      ...
      <div className="">
        <div className="flex justify-between items-center mb-3">
          <p className='text-black font-medium'>
            ...
            </span> - {isNewInvoice ? 'Create Invoice' : 'Edit Invoice'}
          </p>
      ...
      value={
        item.procedureId
          ? {
              ...
              label: item.procedure || proceduresList.find(p => p._id === item.procedureId)?.name || 'Select Procedure',
