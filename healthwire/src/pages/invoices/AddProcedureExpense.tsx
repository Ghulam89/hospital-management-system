...
// Add interfaces for props and local expense
...
interface AddProcedureExpenseProps {
  isModalOpen: boolean;
  ...
  categories: Category[];
  invoiceId?: string;
  onLocalExpenseAdd: (expense: ExpenseFormData) => void;
}

const AddProcedureExpense: React.FC<AddProcedureExpenseProps> = ({ isModalOpen, setIsModalOpen, selectedExpense, fetchExpenses, categories, invoiceId, onLocalExpenseAdd }) => {
  ...
  const handleSelectChange = (name: keyof ExpenseFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [name]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // If no invoiceId and not editing, save locally instead of API
      if (!invoiceId && !selectedExpense) {
        onLocalExpenseAdd({ ...formData, _id: Date.now().toString() }); // Generate temp id
      ...
      if (invoiceId) formDataToSend.append('invoiceId', invoiceId);
      ...
      if (res.data.status === 'ok') {
        ...
        setIsModalOpen(false);
        fetchExpenses?.(); // Call fetchExpenses if provided
      } else {
  ...
  return (
    <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} className="max-w-3xl w-full">
